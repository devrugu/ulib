...
std::variant<int, double, float, std::string, bool> TxtInterpreter::SpecifyDataType(Variable Var)
{
    if (Var.type == "int") {
        ...
        if(Var.type.compare(0, 5, "array") == 0) {
            std::cerr << Var.name << ":" << Var.type << " datatype is unknown.(array is not supported. use vector instead.)\n";
        }
        return "unknown";
    }
}
...
std::unordered_map<std::string, std::any> TxtInterpreter::InterpreteTxt(std::vector<Variable> Variables)
{
    ...
    std::regex VecPattern(R"((vector)<(\w+)>)");
    std::smatch MatchArrVec;

    for (const auto& Var : Variables) {
        if (Var.type.compare(0, 6, "vector") == 0) {
            if (std::regex_search(Var.type, MatchArrVec, VecPattern)) {
                ...
                if (containerType == "vector") {
                    if (elementType == "int") {
                        ...
                        if(Var.type.compare(0, 5, "array") == 0) {
                            std::cerr << Var.name << ":" << Var.type << " datatype is unknown.(array is not supported. use vector instead.)\n";
                            CastedVariables[Var.name] = "unknown";
                        }
                    }
                }
            } else {
                std::cerr << "vector format is incorrect\n";
                exit(1);
            }
        } else {
